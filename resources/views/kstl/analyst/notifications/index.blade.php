{{-- resources/views/kstl/analyst/notifications/index.blade.php --}}
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px;">
                    <div style="display:flex;align-items:center;gap:20px;">
                        ...
                        <div>
                            ...
                            <p style="font-size:12px;color:#94a3b8;margin:0;">Director queries, new tests, and lab updates</p>
                        </div>
                    </div>
                    <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
                        @if(\DB::table('notifications')->where('notifiable_type', 'App\\Models\\User')
                            ...
                            <form method="POST" action="{{ route('analyst.notifications.read-all') }}">
                                @csrf
                                <button type="submit"
                    ...
                    @foreach($notifications as $notification)
                        @php
                            ...
                            $iconColors = match($notifType) {
                                'director_query'    => ['border' => '#dc2626', 'bg' => '#fef2f2', 'color' => '#991b1b'],
                                'new_tests_queued'  => ['border' => '#1a2f4e', 'bg' => '#eff6ff', 'color' => '#1e40af'],
                        ...
                        <div style="background:#fff;border:1px solid {{ $isUnread ? '#bfdbfe' : '#e2e8f0' }};border-left:4px solid {{ $iconColors['border'] }};border-radius:4px;padding:16px 20px;display:flex;align-items:flex-start;gap:16px;">
                            ...
                            @if($isUnread)
                                <form method="POST"
                                      action="{{ route('analyst.notifications.read', $notification->id) }}"
                                      style="flex-shrink:0;">
                                    @csrf
                                    <button type="submit"
