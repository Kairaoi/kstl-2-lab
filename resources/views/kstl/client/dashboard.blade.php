...
<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            {{-- ── Rejected Sample — Consent Required ──────────────────────── --}}
                    ...
                    <div class="px-5 py-4 flex items-start gap-3 border-b border-red-100">
                        ...
                        <div>
                            <p class="text-sm font-semibold text-red-800">
                                Action Required — Sample{{ $pendingConsents->count() > 1 ? 's' : '' }} Rejected by the Lab
                            </p>
                            <p class="text-xs text-red-700 mt-0.5">
                                {{ $pendingConsents->count() > 1 ? 'Some of your samples' : 'One of your samples' }}
                                did not pass the laboratory assessment. Your consent is required before any testing can proceed.
                            </p>
                        </div>
                    </div>
                    <div class="divide-y divide-red-100">
                        @foreach($pendingConsents as $consent)
                            ...
                            <div class="px-5 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                                <div>
                                    <p class="text-sm font-semibold text-red-900">{{ $sample->common_name }}</p>
                                    ...
                                    @if($consent->rejection_reason)
                                        <p class="text-xs text-red-700 mt-1 italic leading-snug max-w-lg">
                                            "{{ $consent->rejection_reason }}"
                                        </p>
                                    @endif
                                    <p class="text-xs text-red-600 mt-1.5 font-medium">
                                        Please indicate whether you consent to proceed with testing despite this rejection, or wish to withdraw your application.
                                    </p>
                                </div>
