{{-- resources/views/kstl/client/notifications/edit.blade.php --}}
<x-app-layout>
    <x-slot name="header">
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px;">
                    <div style="display:flex;align-items:center;gap:20px;">
                        ...
                        <div>
                            ...
                            <h1 style="font-family:'Georgia',serif;font-size:22px;font-weight:700;color:#fff;margin:0 0 6px;line-height:1.2;">Edit Notification</h1>
    ...
    <div style="background:#f1f5f9;min-height:100vh;padding:52px 0 56px;">
        <div style="max-width:56rem;margin:0 auto;padding:0 2rem;">
            ...
            <form method="POST" action="{{ route('client.notifications.read', $notification->id ?? 0) }}">
                @csrf
                @method('PUT')
                        ...
                        <div style="display:grid;gap:20px;">
                            ...
                            <div>
                                ...
                                <select name="type"
                                    ...
                                    <option value="action_required" {{ old('type', $notification->type ?? '') === 'action_required' ? 'selected' : '' }}>Action Required</option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="message" rows="6" required
                                          style="width:100%;padding:8px 12px;border:1px solid #cbd5e1;border-radius:3px;font-size:13px;color:#1e293b;background:#fff;resize:vertical;box-sizing:border-box;">{{ old('message', $notification->message ?? $notification->data['message'] ?? '') }}</textarea>
                            </div>
                ...
                @isset($notification)
                    <div style="background:#fff;border:1px solid #e2e8f0;border-radius:4px;overflow:hidden;margin-bottom:24px;">
                        <div style="padding:16px 24px;border-bottom:1px solid #e2e8f0;">
                            <h3 style="font-family:'Georgia',serif;font-size:14px;font-weight:700;color:#1a2f4e;margin:0;">Status</h3>
                        </div>
                        <dl style="padding:0 24px;margin:0;">
                            <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid #f1f5f9;">
                                <dt style="font-size:12px;color:#64748b;">Created</dt>
                                <dd style="font-size:13px;color:#374151;margin:0;">{{ $notification->created_at?->format('d M Y \a\t H:i') }}</dd>
                            </div>
                            <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;">
                                <dt style="font-size:12px;color:#64748b;">Read At</dt>
                                <dd style="font-size:13px;margin:0;{{ $notification->read_at ? 'color:#374151;' : 'color:#94a3b8;font-style:italic;' }}">
                                    {{ $notification->read_at?->format('d M Y \a\t H:i') ?? 'Not yet read' }}
                                </dd>
                            </div>
                        </dl>
                    </div>
