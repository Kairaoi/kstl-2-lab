                        ...
                        <div style="padding:20px 24px;display:flex;flex-direction:column;gap:20px;">
                            ...
                            <div>
                                ...
                                <textarea id="sample_description"
                                          ...
                                          placeholder="Describe the sample...">{{ old('sample_description', $submission->sample_description) }}</textarea>
                            ...
                            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                                <div>
                                    ...
                                    <select id="sample_type"
                                        ...
                                        <option value="fish"      {{ old('sample_type', $submission->sample_type) === 'fish'      ? 'selected' : '' }}>Fish</option>
                                        <option value="shellfish" {{ old('sample_type', $submission->sample_type) === 'shellfish' ? 'selected' : '' }}>Shellfish</option>
                                        <option value="seaweed"   {{ old('sample_type', $submission->sample_type) === 'seaweed'   ? 'selected' : '' }}>Seaweed</option>
                                        <option value="water"     {{ old('sample_type', $submission->sample_type) === 'water'     ? 'selected' : '' }}>Water</option>
                                        <option value="sediment"  {{ old('sample_type', $submission->sample_type) === 'sediment'  ? 'selected' : '' }}>Sediment</option>
                                        <option value="other"     {{ old('sample_type', $submission->sample_type) === 'other'     ? 'selected' : '' }}>Other</option>
                                    </select>
                                ...
                                <div>
                                    ...
                                    <div style="display:flex;">
                                        <input id="sample_quantity"
                                               ...
                                               value="{{ old('sample_quantity', $submission->sample_quantity) }}"
                                               ...
                                               step="0.01"
                                               required>
                                        @php $sqUnit = old('sample_quantity_unit', $submission->sample_quantity_unit ?? 'kg'); @endphp
                                        <select name="sample_quantity_unit"
                                            ...
                                            <option value="g"  {{ $sqUnit === 'g'  ? 'selected' : '' }}>g</option>
                                            <option value="kg" {{ $sqUnit === 'kg' ? 'selected' : '' }}>kg</option>
                                            <option value="ml" {{ $sqUnit === 'ml' ? 'selected' : '' }}>ml</option>
                                            <option value="L"  {{ $sqUnit === 'L'  ? 'selected' : '' }}>L</option>
                                        </select>
                                    </div>
                            ...
                            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                                <div>
                                    ...
                                    <input id="collected_at"
                                           type="date"
                                           name="collected_at"
                                           value="{{ old('collected_at', $submission->collected_at?->format('Y-m-d')) }}"
                                           style="width:100%;padding:8px 12px;border:1px solid #cbd5e1;border-radius:3px;font-size:13px;color:#1e293b;background:#fff;box-sizing:border-box;"
                                           max="{{ date('Y-m-d') }}"
                                           required>
                    ...
                    <div style="background:#fff;border:1px solid #e2e8f0;border-radius:4px;overflow:hidden;">
                        <div style="padding:20px 24px;display:flex;flex-direction:column;gap:20px;">
                                    ...
                                    @foreach(['routine' => 'Routine', 'urgent' => 'Urgent'] as $value => $label)
                                        <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13px;color:#374151;">
                                            <input type="radio"
                                                   name="priority"
                                                   value="{{ $value }}"
                                                   {{ old('priority', $submission->priority ?? 'routine') === $value ? 'checked' : '' }}>
                                            {{ $label }}
                                        </label>
                                    @endforeach
                                </div>
                            ...
                            <div>
                                ...
                                @php $storageCond = old('storage_conditions', $submission->storage_conditions); @endphp
                                <select id="storage_conditions"
                                        name="storage_conditions"
                                        style="width:100%;padding:8px 12px;border:1px solid #cbd5e1;border-radius:3px;font-size:13px;color:#1e293b;background:#fff;">
                                    <option value="">— Select conditions —</option>
                                    <option value="ambient"  {{ $storageCond === 'ambient'  ? 'selected' : '' }}>Ambient / Room temperature</option>
                                    <option value="chilled"  {{ $storageCond === 'chilled'  ? 'selected' : '' }}>Chilled (0–4°C)</option>
                                    <option value="frozen"   {{ $storageCond === 'frozen'   ? 'selected' : '' }}>Frozen (−18°C or below)</option>
                                    <option value="dry_ice"  {{ $storageCond === 'dry_ice'  ? 'selected' : '' }}>Dry Ice</option>
                                </select>
