{{-- resources/views/kstl/director/notifications/index.blade.php --}}
                ...
                <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px;">
                    ...
                    <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
                        @if(\DB::table('notifications')->where('notifiable_type', 'App\\Models\\User')
                            ...
                            <form method="POST" action="{{ route('director.notifications.read-all') }}">
                                @csrf
                                <button type="submit"
                    ...
                    @foreach($notifications as $notification)
                        @php
                            ...
                            $iconColors = match($notifType) {
                                ...
                                'payment_submitted'      => ['border' => '#16a34a', 'bg' => '#f0fdf4', 'color' => '#166534'],
                                'complaint_received'     => ['border' => '#dc2626', 'bg' => '#fef2f2', 'color' => '#991b1b'],
                        ...
                        <div style="background:#fff;border:1px solid {{ $isUnread ? '#bfdbfe' : '#e2e8f0' }};border-left:4px solid {{ $iconColors['border'] }};border-radius:4px;padding:16px 20px;display:flex;align-items:flex-start;gap:16px;">
                            ...
                            @if($isUnread)
                                <form method="POST"
                                      action="{{ route('director.notifications.read', $notification->id) }}"
                                      style="flex-shrink:0;">
                                    @csrf
                                    <button type="submit"
